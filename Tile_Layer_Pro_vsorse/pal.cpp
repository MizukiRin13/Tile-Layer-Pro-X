...
if (MainForm->ActiveForm == NULL)
    ...
    {
    if ((ClipForm->ClipEntry == NULL)
        || (ClipForm->ClipEntry->data == NULL))
        return;
    int maxpal = (256 / (1 << ClipForm->ClipEntry->bpp)) - 1;
    if (Button == btNext)
        {
        if (ClipForm->ClipEntry->pal == maxpal)
            ClipForm->ClipEntry->pal = 0;
        else
            ClipForm->ClipEntry->pal++;
        }
    else
        {
        if (ClipForm->ClipEntry->pal == 0)
            ClipForm->ClipEntry->pal = maxpal;
        else
            ClipForm->ClipEntry->pal--;
        }
    }
